...
int gmy2lets(char *fname_gmy, char *fname_inlets, bool find_inlets)
{
  // gmy file handle
  FILE *gmyFile;
...
int main(int argc, char **argv)
{
  if (argc == 4) {
    ...
    int ret = gmy2lets(fname_gmy, fname_inlets, find_inlets);
    if (ret != 1) {
      return 1;
    }
    return 0;
  ...
  /* otherwise, report usage */
    ...
    fputs("Usage: gmy2lets infile.gmy outfile.inlets doInlets\n", stderr);
    return 1;
  }
}
