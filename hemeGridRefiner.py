...
BASE="/net/storeptr1/heme/HemePure_JM/HemePure_tools/"
VOXELIZERPATH =BASE+"voxelizer/source/voxelizer3"#_file_ARCHER"
MAKEGMYMPIPATH =BASE+"vx2gmy/make_gmy_MPI.sh"
GMY2INLETSPATH =BASE+"gmy2inlets/gmy2inlets"
VXREFINERPATH =BASE+"vx2refine/vx2refine"
VXPARAVIEWPATH =BASE+"vx2paraview/vx2paraview"
VXASCIIPATH =BASE+"vx2ascii/vx2ascii"
...
with open("iolets_block_inputxml_Ref1.txt", 'w') as f:
    f.writelines( lines )
...
for cycles in range(1,REFINES):
    ...
    with open("iolets_block_inputxml_Ref" + str(cycles) + ".txt", "r") as f:
        lines = f.readlines()
        j=0
        for i in lines:
            i = i.split()
    ...
    with open("iolets_block_inputxml_Ref" + str(cycles+1) + ".txt", 'w') as f:
        f.writelines( lines )

# Get the inlets and outlets xml blocks (for the hemelb input xml) output by the voxelizer
with open("iolets_block_inputxml_Ref"+str(REFINES)+".txt", "r") as ioletsblockfile:
...
execute(VXASCIIPATH + " fluidAndLinks_Ref" + str(REFINES) + ".dat > " + ROOTNAME + "RefCHECK.txt") 
